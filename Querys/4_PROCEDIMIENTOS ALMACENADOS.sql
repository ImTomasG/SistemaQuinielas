...
GO
IF EXISTS (SELECT * FROM sys.objects WHERE TYPE = 'p' AND name = 'SP_PREMIOSFINALES')
DROP PROCEDURE SP_PREMIOSFINALES
GO
...
/*AL FINALIZAR EL TORNEO SE DIVIDE EL TOTAL: 50% PARA PRIMER LUGAR, 25% PARA SEGUNDO LUGAR, 10% PARA EL TERCER LUGAR, 10% PARA EL ÚLTIMO LUGAR.*/
CREATE PROC SP_PREMIOSFINALES(
@ID_LIGA INTEGER
)AS

--SE DECLARA LA VARIABLE QUE ALMACENARÁ LA REPARTICIÓN DEL 50% PARA EL PRIMER LUGAR DE LAS LIGAS CONFIGURADAS COMO TIPO DE APUESTAS
DECLARE @PRIMER_LUGAR NUMERIC (10,2)
--SE DECLARA LA VARIABLE QUE ALMACENARÁ LA REPARTICIÓN DEL 25% PARA EL SEGUNDO LUGAR DE LAS LIGAS CONFIGURADAS COMO TIPO DE APUESTAS
DECLARE @SEGUNDO_LUGAR NUMERIC (10,2)
--SE DECLARA LA VARIABLE QUE ALMACENARÁ LA REPARTICIÓN DEL 10% PARA EL TERCER LUGAR DE LAS LIGAS CONFIGURADAS COMO TIPO DE APUESTAS
DECLARE @TERCER_LUGAR NUMERIC (10,2)
--SE DECLARA LA VARIABLE QUE ALMACENARÁ LA REPARTICIÓN DEL 10% PARA EL ÚLTIMO LUGAR DE LAS LIGAS CONFIGURADAS COMO TIPO DE APUESTAS
DECLARE @ULTIMO_LUGAR NUMERIC (10,2)

--SE ASIGNA EL 50% DE UNA LIGA DETERMINADA QUE SE ENCUENTRA CONFIGURADA COMO TIPO APUESTA
SELECT @PRIMER_LUGAR = ((SUM(P.MONTO)*0.5)) FROM LIGA L JOIN USUARIOLIGA UL ON L.ID_LIGA = UL.ID_LIGA JOIN PAGO P ON UL.ID_USUARIO = P.ID_USUARIO AND UL.ID_LIGA = P.ID_LIGA AND UL.ID_ROL = P.ID_ROL WHERE L.ID_LIGA = @ID_LIGA AND L.ID_TIPOLIGA = 1
--SE ASIGNA EL 25% DE UNA LIGA DETERMINADA QUE SE ENCUENTRA CONFIGURADA COMO TIPO APUESTA
SELECT @SEGUNDO_LUGAR = ((SUM(P.MONTO)*0.25)) FROM LIGA L JOIN USUARIOLIGA UL ON L.ID_LIGA = UL.ID_LIGA JOIN PAGO P ON UL.ID_USUARIO = P.ID_USUARIO AND UL.ID_LIGA = P.ID_LIGA AND UL.ID_ROL = P.ID_ROL WHERE L.ID_LIGA = @ID_LIGA AND L.ID_TIPOLIGA = 1
--SE ASIGNA EL 10% DE UNA LIGA DETERMINADA QUE SE ENCUENTRA CONFIGURADA COMO TIPO APUESTA
SELECT @TERCER_LUGAR = ((SUM(P.MONTO)*0.10)) FROM LIGA L JOIN USUARIOLIGA UL ON L.ID_LIGA = UL.ID_LIGA JOIN PAGO P ON UL.ID_USUARIO = P.ID_USUARIO AND UL.ID_LIGA = P.ID_LIGA AND UL.ID_ROL = P.ID_ROL WHERE L.ID_LIGA = @ID_LIGA AND L.ID_TIPOLIGA = 1
--SE ASIGNA EL 10% DEL ÚLTIMO LUGAR DE UNA LIGA DETERMINADA QUE SE ENCUENTRA CONFIGURADA COMO TIPO APUESTA
SELECT @ULTIMO_LUGAR = ((SUM(P.MONTO)*0.10)) FROM LIGA L JOIN USUARIOLIGA UL ON L.ID_LIGA = UL.ID_LIGA JOIN PAGO P ON UL.ID_USUARIO = P.ID_USUARIO AND UL.ID_LIGA = P.ID_LIGA AND UL.ID_ROL = P.ID_ROL WHERE L.ID_LIGA = @ID_LIGA AND L.ID_TIPOLIGA = 1
	...
	BEGIN
		...
		(
			...
			JOIN LIGA L ON UL.ID_LIGA = L.ID_LIGA
			JOIN APUESTA A ON UL.ID_USUARIO = A.ID_USUARIO AND UL.ID_LIGA = A.ID_LIGA AND UL.ID_ROL = A.ID_ROL
			WHERE L.ID_LIGA = @ID_LIGA AND L.ID_TIPOLIGA = 1

			GROUP BY L.ID_LIGA,L.NOMBRE_LIGA,U.ID_USUARIO,U.NOMBRES,U.APELLIDOS
		)
		SELECT *, CASE WHEN P.POSICIONFILAS = 1 THEN @PRIMER_LUGAR WHEN P.POSICIONFILAS = 2 THEN @SEGUNDO_LUGAR WHEN P.POSICIONFILAS = 3 THEN @TERCER_LUGAR WHEN P.POSICIONFILAS = 5 THEN @ULTIMO_LUGAR ELSE '0.00' END AS 'PREMIO_CONSEGUIDO'
		FROM POSICIONES P;
	END
